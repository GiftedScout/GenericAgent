...
_THINK_PAIR2_RE = re.compile(r"<think>.*?</think>\s*", re.DOTALL)
...
def _preclean_display_impl(text: str, compact_tools: bool, allow_open_window: bool = True) -> str:
    ...
    # 配对剥除（非贪婪，输入已由 llmcore 源头转义内嵌字面标签）
...
_TIPS = (
    ...
    "Tip: /update 同步上游和 myfork；冲突时展示 diff 并请求选择，完成后简要汇报；/autorun 进入 autonomous 自主模式。",
